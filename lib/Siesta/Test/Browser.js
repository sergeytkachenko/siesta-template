...
Class('Siesta.Test.Browser', {
    ...
    // Rendering engine detection
    if (t.bowser.webkit) { .. do something .. }
    ...
    methods : {
        ...
        launch : function () {
            ...
            if (!me.parent) {
                ...
                if (!me.harness.browserWindowHasFocus() && !bowser.safari) me.onWindowBlur()
...
//                })
                ...
                if (!me.parent && !bowser.msie && me.harness.showCursor && !win.opener) {
                    me.mouseVisualizer = new Siesta.Harness.Browser.UI.MouseVisualizer();
                    me.mouseVisualizer.beginCursorVisualization(me.global);
                }
            }
...
//            // EOF IGNORE
        ...
        sizzle : function (selector, root) {
            return Sizzle(selector, root || this.global.document)
        },
        ...
        scrollTargetIntoView : function (target, offset) {
            if (this.typeOf(target) != 'Array') {
                ...
                div.style.cssText   = 'position : absolute; left: ' + target[ 0 ] + 'px; top: ' + target[ 1 ] + 'px'
